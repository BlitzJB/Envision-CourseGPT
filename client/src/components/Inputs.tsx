...
export const Inputs = ({ topic, setTopic, stage, setStage, setOutline, outline }: InputsProps) => {
    ...
    const handleGenerateOutline = async (): Promise<Outline> => {
        ...
        const response = await fetch(`${BASEURL}/api/get_outline`, {
            method: "POST", 
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    const handleGenerateSubheading = async () => {
        ...
        const eventSource = new EventSource(`${BASEURL}/api/get_all_subtopics?outline=${encodedOutline}&topic=${topic}`)
        ...
        eventSource.onmessage = (event) => {
            ...
            setOutline((prevOutline: Outline) => {
                ...
                newOutline.items[data.topic_number-1].subtopics[data.subtopic_number-1].text = data.content
